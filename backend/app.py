...
def parse_voice():
    """
    Takes natural language text (Hindi, Kannada, English, etc.) and extracts
    structured demographic data using Gemini.
    """
    user_input = request.get_json(silent=True)
    if not user_input or 'text' not in user_input:
        return jsonify({"status": "error", "message": "Missing 'text' in body."}), 400

    text = user_input['text'].strip()
    if not text:
        return jsonify({"status": "empty", "data": {}}), 200

    if not GEMINI_API_KEY:
        return jsonify({"status": "error", "message": "Gemini API key missing on backend."}), 503

    prompt = f"""
You are an expert NLP data extractor.
Extract the following user parameters from the provided text.
If a parameter is not explicitly mentioned, estimate smartly or fallback to defaults.
Text: "{text}"

Parameters to extract:
1. "age" (integer): Age in years. If not mentioned but they seem young adult, assume 25. If completely unknown, return 35.
2. "income" (float): Annual income in Rupees. If monthly is given, multiply by 12. If unknown, assume 50000.
3. "land_size" (float): Agricultural land size in Acres. (1 hectare ~ 2.47 acres). If unknown, return 0.0.
4. "is_rural" (1 or 0): 1 if rural/village, 0 if urban/city. If farmer or agriculture referenced, assume 1. Default 1.
5. "house_type" (1 or 0): 1 if Pucca (concrete) house, 0 if Kutcha (mud/temporary) house. "pakka makan nahi hai" means 0. Default 0.
6. "is_taxpayer" (1 or 0): 1 if they pay income tax, 0 otherwise. Default 0.
7. "is_govt_emp" (1 or 0): 1 if government employee, 0 otherwise. Default 0.

Output MUST be strictly valid JSON, with exactly these 7 keys, and no extra text or markdown formatting.
Example Output:
{{"age": 25, "income": 70000.0, "land_size": 0.5, "is_rural": 1, "house_type": 0, "is_taxpayer": 0, "is_govt_emp": 0}}
"""
    try:
        client = genai.Client(api_key=GEMINI_API_KEY)
        response = client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
        ...
        logger.error(f"Voice Parsing NLP error: {e}", exc_info=True)
        return jsonify({"status": "error", "message": "Failed to parse text via NLP."}), 500
